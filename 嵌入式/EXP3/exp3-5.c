...
int i=0;
char receive[50]; //ÕâÀïÏÞÖÆÁË´®¿Ú×î¶à½ÓÊÕ50¸ö×Ö½Ú
const char *ATCLASS="AT+CLASS";        //Ô¤ÏÈ¶¨Òå·¢ËÍ½ÓÊÕÐ­Òé£¬·¢ËÍ×Ö·û´®ÒÔ¡®#¡¯½áÊø¡£
const char *ATCLASS_small="at+class";        //Ô¤ÏÈ¶¨Òå·¢ËÍ½ÓÊÕÐ­Òé£¬·¢ËÍ×Ö·û´®ÒÔ¡®#¡¯½áÊø¡£
const char *ATCODE="AT+STUDENTCODE";
const char *ATCODE_small="at+studentcode";
...
//ÏÂÃæµÄº¯Êý¿ÉÒÔ½ÓÊÕ³¬³ö½ÓÊÕFIFOÖÐ¶Ï´¥·¢Éî¶ÈµÄ´®ÐÐÊý¾Ý£¬µ«×î³¤<50×Ö½Ú
void UARTStringGetNonBlocking(char *cMessage,int32_t uart0_int_status)
{
	if(uart0_int_status==0x10)   //Õý³£µÄUART½ÓÊÕÖÐ¶Ï
	{
		while(UARTCharsAvail(UART0_BASE))
		{
			cMessage[i]=UARTCharGetNonBlocking(UART0_BASE);  //iÎª¶¨ÒåµÄÈ«¾Ö±äÁ¿£¬´ú±íUART½ÓÊÕ×Ö·ûÊý×éreceive[]µ±Ç°µÄÊý×éÏÂ±ê
			i++;
		}
		if(cMessage[i-1]=='S'){    //Õý³£½ÓÊÕÖÐ¶Ï½áÊø£¬ÅÐ¶Ï×îºóÒ»¸ö×Ö·ûÊÇ·ñÎª#ºÅ½áÊø·û£¬ÈôÊÇÔòÏÂ±êÇåÁã¡£
			cMessage[i]='\0';
			i=0;
		}
	}
	if(uart0_int_status==0x40)   //³¬Ê±½ÓÊÕÖÐ¶Ï
	{
	    ...
	    cMessage[i]='\0';         //³¬Ê±½ÓÊÕÖÐ¶Ï½áÊø£¬ÎÞÐèÅÐ¶Ï×îºóÒ»¸ö×Ö·ûÊÇ·ñÎª#£¬ÒòÎª³¬Ê±ÖÐ¶ÏÒ»¶¨ÒâÎ¶×Å±¾´Î½ÓÊÕ½áÊø¡£
		i=0;
	}
}
...
void S800_UART_Init(void)
{
  ...
  UARTConfigSetExpClk(UART0_BASE, ui32SysClock,115200,(UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE |UART_CONFIG_PAR_NONE));
	...
	//UART0µÄFIFOµÄ·¢ËÍºÍ½ÓÊÕÖÐ¶ÏµÄ´¥·¢Ë®Æ½¾ù±»ÉèÖÃÎª6/8£¬Ò²¼´´¥·¢Éî¶ÈÎª12¸ö×Ö½Ú¡£±¾³ÌÐòÖÐÎ´Ê¹ÓÃ·¢ËÍÖÐ¶Ï¡£
...
void UART0_Handler(void)			//1.clear the status 2.getstring(nonblocking) 3.judge the string the interrupt 4.gpiopinwrite
{
	...
	//Ô¤ÉèµÄ½ÓÊÕÖÐ¶Ï´¥·¢Éî¶ÈÎª12×Ö½Ú¡£
	//AT+CLASSÎª8×Ö½Ú£¬Òò´Ë½ÓÊÕÊ±£¬²úÉú³¬Ê±ÖÐ¶Ï½øÈë±¾º¯Êý¡£
	//AT+STUDENTCODEÎª14×Ö½Ú£¬½ÓÊÕÊ±£¬Ç°12×Ö½Ú´¥·¢½ÓÊÕÖÐ¶Ï£¬ºóÁ½×Ö½ÚÔÙ²úÉúÒ»¸ö³¬Ê±ÖÐ¶Ï¡£»á½øÈë±¾º¯ÊýÁ½´Î¡£
	//Òò´ËÔÚÔ¤Éè12×Ö½Ú´¥·¢Éî¶ÈÏÂ£¬Á½ÖÖÖ¸Áî×Ö·û´®½ÓÊÕÍê±ÏÊ±µÄÖÐ¶ÏÀàÐÍ¾ùÎª³¬Ê±ÖÐ¶Ï¡£

	if (uart0_int_status==0x40)
	{
	  if(strcmp(receive,ATCLASS)==0||strcmp(receive,ATCLASS_small)==0)   UARTStringPutNonBlocking(CLASS);
		if(strcmp(receive,ATCODE)==0||strcmp(receive,ATCODE_small)==0)    UARTStringPutNonBlocking(CODE);
  }

...
int main(void)
{
  ...
  IntMasterEnable();		
	...
	while (1)
	{
		...
		{
			if(strncmp(Stringget,month[count],3)==0)
			{
				flag=1;
				break;
			}
		...
		if(flag)
		{
			int number=(Stringget[4]-'0')*10+(Stringget[5]-'0');
		
			switch(Stringget[3])
			{
				case '+':count=(count+number)%12;break;
				default:count=(count-number);if(count<0){count=count+12;}break;
			}
...
void show(int count)
{
	sprintf(Stringput,"%s",month[count]);
